...
repository (no remote theme). **The `gh-pages` branch is the source of truth**;
`main` holds only the licence and this pointer. GitHub Actions builds
`gh-pages` and deploys the result to Pages, so pushing to `gh-pages` publishes.
